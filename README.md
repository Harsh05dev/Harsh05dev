<div align="center">

<!-- Typing SVG -->
<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=28&duration=3000&pause=1000&color=58A6FF&center=true&vCenter=true&multiline=true&repeat=true&width=600&height=100&lines=Hey+there!+I'm+Harsh+Mehta+%F0%9F%91%8B;AI+%7C+ML+%7C+Full+Stack+%7C+Builder" alt="Typing SVG" /></a>

<!-- Profile Views -->
<!-- <img src="https://komarev.com/ghpvc/?username=Harsh05dev&label=Profile%20Views&color=58a6ff&style=for-the-badge" alt="Profile Views" /> -->

</div>

---

## 🧠 About Me

```yaml
name: Harsh Mehta
location: New Jersey, US
education: BS/MS Computer Science & AI @ NJIT (Class of 2027)
gpa: 3.91
current_roles:
  - AI/ML Research Intern @ NJIT (NASA-funded Space Weather project)
  - AI Training Fellow @ Handshake
  - Substitute Teacher @ Scoot Edu
  - Program Instructor @ JerseySTEM
interests: ["AI/ML", "Full Stack Dev", "Table Tennis", "Guitar"]
```

- 🚀 Accepted into AI/ML research at NJIT under a NASA-funded Space Weather project
- 🔭 Currently building ML products and full-stack dashboards for real-world impact
- 🏓 Table tennis player and guitar enthusiast
- 📫 Reach me at: [LinkedIn](https://www.linkedin.com/in/harsh-mehta-njit/) · `harshmehta1505@gmail.com`

---

## ⚡ Tech Stack

| Category | Stack |
|---|---|
| **Languages** | <img src="https://skillicons.dev/icons?i=python,js,java,php,mysql&theme=dark" /> |
| **Frontend** | <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,bootstrap,html,css,js&theme=dark" /> |
| **Backend** | <img src="https://skillicons.dev/icons?i=nodejs,express,supabase&theme=dark" /> |
| **ML / AI** | <img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,jupyter&theme=dark" /> |
| **Tools / Platforms** | <img src="https://skillicons.dev/icons?i=git,github,npm,vercel,linux,vscode,gcp,mongodb,figma,notion&theme=dark" /> |

---

## 🏆 Highlight

Placed **2nd in Track B** at the **Morgan Stanley Code to Give Hackathon (NYC)** with Team 8.

Built **Grove**, a data fusion platform for LemonTree Food Helpline that supports:
- Government analysts (coverage gap analysis and food desert mapping)
- Pantry/resource operators (benchmarking and reliability tools)
- Donors/funders (funding simulator and impact visibility)

Out of 2,200+ applicants, ~100 participants were selected.

---

## 🚀 Featured Projects

<div align="center">

[![BS/MS Pathway Planner](https://github-readme-stats.vercel.app/api/pin/?username=Harsh05dev&repo=ywcc-bsms-planner&theme=tokyonight&hide_border=true)](https://github.com/Harsh05dev/ywcc-bsms-planner)
[![Snowstorm Flight Delay Predictor](https://github-readme-stats.vercel.app/api/pin/?username=Harsh05dev&repo=snowstorm-flight-delay-predictor&theme=tokyonight&hide_border=true)](https://github.com/Harsh05dev/snowstorm-flight-delay-predictor)
[![Morgan Stanley Team 8 Grove](https://github-readme-stats.vercel.app/api/pin/?username=Harsh05dev&repo=MorganStanley_Team8_Grove&theme=tokyonight&hide_border=true)](https://github.com/Harsh05dev/MorganStanley_Team8_Grove)

</div>

### 1) BS/MS Pathway Planner
BS/MS Pathway Planner for NJIT's Ying Wu College of Computing.  
**Stack:** React, FastAPI, PostgreSQL, Supabase  
**Demo:** [ywcc-bsms-planner.vercel.app](https://ywcc-bsms-planner.vercel.app)

### 2) Snowstorm Flight Delay Predictor
ML dashboard to predict EWR winter flight delays using weather + airline features.  
**Stack:** Python, scikit-learn, XGBoost, Streamlit  
**Result:** XGBoost ROC-AUC **0.732**  
**Demo:** [Streamlit app](https://snowstorm-flight-delay-predictor-kms4suw7khir8j7gjkncjp.streamlit.app/)

### 3) Grove (Morgan Stanley Code to Give 2026)
Multi-persona analytics platform for NYC food access partners, integrating pantry, ACS, and ALICE data.  
**Stack:** Next.js, Express, Supabase, Gemini, Anthropic  
**Repo:** [MorganStanley_Team8_Grove](https://github.com/Harsh05dev/MorganStanley_Team8_Grove)

---

## 🤝 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/harsh-mehta-njit/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Harsh05dev)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:harshmehta1505@gmail.com)

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer" width="100%" />

</div>
